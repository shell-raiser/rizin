...
static bool is_lea(const char *asm_str) {
	...
	RzRegex *re = rz_regex_new("(^\x1b\\[\\d{1,3}mlea\x1b\\[0m.+)", RZ_REGEX_EXTENDED | RZ_REGEX_CASELESS);
	if (!re) {
		return false;
	}
	bool res = rz_regex_match(re, asm_str, 0, RZ_REGEX_EXTENDED | RZ_REGEX_CASELESS, NULL) != RZ_REGEX_ERROR_NOMATCH;
	rz_regex_free(re);
	return res;
}
